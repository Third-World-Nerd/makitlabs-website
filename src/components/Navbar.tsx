...
const Navbar: React.FC = () => {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <a
      href={href}
      className="text-lg hover:text-purple-600 transition duration-300" // Assuming 'primary' is purple-600
  ...
  return (
    ...
    <nav className="fixed z-50 w-full bg-white/95 backdrop-blur-md font-default shadow-lg">
      <div className="flex h-20 items-center justify-between px-[4%]"> {/* Ensure header height is adequate */}
   ...
   <img src="/photo/logo/MakitLabs_Logo.png" alt="Makit Labs Logo" className=" w-24 sm:w-32 h-auto" />
        ...
        <div className="hidden lg:ml-auto lg:flex lg:items-center lg:space-x-8">
          ...
          <NavLink href="#Products">Products</NavLink> {/* Added #Products for consistent anchor link */}
          
          {/* WhatsApp Link */}
          <a 
            className='text-purple-600 text-xl font-semibold hover:scale-105 cursor-pointer transition duration-300'
        ...
        >
          {isOpen ? (
            <XMarkIcon className='text-purple-600 h-8 w-8'/>
          ) : (
            <Bars3Icon className='text-purple-600 h-8 w-8'/>
          )}
        </button>
      </div>
      ...
      >
        <div className='flex flex-col space-y-6 p-6'>
          ...
          <div className='pt-4 border-t border-gray-100 mt-6'>
             <a 
              className='text-purple-600 text-xl font-semibold hover:scale-105 cursor-pointer transition duration-300 block mb-4'
              href='https://wa.me/9779767470587?text=I%20saw%20your%20website%20and%20want%20to%20know%20more%20about%20your%20services.'
              target='_blank'
              rel="noopener noreferrer"
